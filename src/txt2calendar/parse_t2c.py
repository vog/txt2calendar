...
def parse_record_dict(record):
    ...
    for field in record:
        ...
        if field_name in record_dict:
            ...
            record_dict[alternative_field_name] = value_lines
        else:
            record_dict[field_name] = value_lines
    return record_dict

def parse_event_date(value_lines):
...
def parse_event_mott(value_lines):
    return ' '.join(value_lines)

def parse_event_tags(value_lines):
    return set(
        tag
...
def parse_event_titl(value_lines):
    return value_lines
...
def parse_event(record):
    ...
    recognized_fields = {
        field_name: field_parser(record_dict.get(field_name, []))
    ...
    unrecognized_fields = sorted(
        ...
        for field_name, value_lines in record_dict.iteritems()
...
def parse_sorted_events(fileobj):
    return sorted(parse_events(fileobj), key=event_sort_key)
